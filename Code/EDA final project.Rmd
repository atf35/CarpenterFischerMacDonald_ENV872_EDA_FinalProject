...
Higher specific conductivity is correlated with higher concentrations of each salt, as is expected since more salt ions leads to higher salinity. However, there is no discernible pattern related to the DOY. While our time series analysis showed a seasonal component to 
...
```{r}
ggplot(NeuseWQ_DOY) +
  geom_point(aes(x = DOY, y = SpC_uS.cm)) +
  geom_abline(aes(x = DOY, y = SpC_uS.cm))
```
...
saltsvspc_plot <- ggplot(NeuseWQ_DOY)+
  ...
  labs(x = "Spec. Cond.", y = "mg/L", color = "DOY", caption = "Sodium is depicted as the plus, calcium is depicted as the triangle, magnesium is depicted as the circle")
saltsvspc_plot
```
