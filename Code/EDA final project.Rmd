...
We plotted the specific conductance over time to visualize any gaps in our dataset. We see that the WQ dataset contains many periods of missing data for specific conductance. Since these missing periods frequently span across many years, we chose to look at WQ data from 2013 through 2021. There are no missing data points from this period of time, and we will therefore not require any interpolation of this dataset. 
...
Answer: After running the the MannKendall test for seasonality, we have determined that for the Nuese river there is no significant in seasonality for conductivity (p = 0.25276). However, we can conclude seasonality in flow data, discharge varies significantly depending on the season (p = 3.368e-07).
...
#Testing for seasonal trends within conducivity
...
```{r Visualize 2013-2021 WQ data, echo=FALSE, fig.align='left', fig.cap="Sodium in the Neuse River from 2013 through 2021."}
...
```{r Visualize 2013-2021 WQ data, echo=FALSE, fig.align='left', fig.cap="Calcium in the Neuse River from 2013 through 2021."}
...
=======
Ca.plot
>>>>>>> 4cf799b206d7ade0e3054003bf3da16f34afcfae
```

```{r Visualize 2013-2021 WQ data, echo=FALSE, fig.align='left', fig.cap="Magnesium in the Neuse River from 2013 through 2021."}
...
```{r Visualise all three components together, echo=FALSE, fig.align='left'}
...
```{r add on top of seasonality graph, echo= FALSE, fig.align='left'}
#Get everything on the same graph and check for trends
