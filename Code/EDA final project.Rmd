...
After running the the MannKendall test for seasonality, we have determined that for the Nuese river there is no significant seasonality component for conductivity (p = 0.25276). Similarly, there is no significant seasonal component to the data for each salt ion. However, we can conclude that there is significant seasonality in flow data, discharge varies significantly depending on the season (p = 3.368e-07).
...
```{r Plotting by DOY, include=TRUE, echo=FALSE, message=FALSE}
...
saltsvspc_plot <- ggplot(NeuseWQ_DOY)+
  ...
  labs(x = "Spec. Cond.", y = "mg/L", color = "DOY", title = "Salt vs. Specific Conductivity across the Day of Year")
...
The Neuse River at Kinston does not show strong seasonal trends in salinity. Specific conductance, the proxy used to examine salinity, showed marginal seasonal trends when examined using both a time series decomposition and when plotted by the day of the year. Sodium appears to have the strongest correlation with specific conductance, indicating that it perhaps is the main driver of the salinity that does exist in the Neuse River. Magnesium and calcium had a much lower correlation with specific conductance, showing that they do not influence the overall salinity as much as sodium. Overall, this is good news for the Neuse, indicating that at Kinston, which is relatively far down the watershed, upstream uses of salts do not cause seasonal changes in the river. 
